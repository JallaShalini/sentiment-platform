...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/sentiment/distribution`)
      .then((res) => res.json())
      .then((data) =>
        setStats({ total_posts: data.total, distribution: data.distribution || {} })
      );
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <StatCard title="Total Posts" value={stats.total_posts} icon={<MessageSquare />} />
        <StatCard title="Positive" value={stats.distribution.positive} icon={<TrendingUp />} color="text-green-400" />
        <StatCard title="Negative" value={stats.distribution.negative} icon={<AlertTriangle />} color="text-red-400" />
        <StatCard title="Neutral" value={stats.distribution.neutral} icon={<Activity />} color="text-gray-400" />
      </div>
